...
func _physics_process(delta : float) -> void:
	if _can_update_day_phase:
		_update_day_phase(delta)
...
func _update_day_phase(delta : float) -> void:
	_current_day_phase_time += delta
	_update_idle_time += delta
	_current_day_time += delta

	if current_day_phase == DAY_STATE.DAWN:
		if _current_day_phase_time > _dawn_duration:
			current_day_phase = DAY_STATE.DAY
			emit_signal("day_phase_updated", current_day_phase)
	if current_day_phase == DAY_STATE.DAY:
		if _current_day_phase_time > _day_phase_duration + _dawn_duration:
			current_day_phase = DAY_STATE.DUSK
			_current_day_phase_time = 0.0
			emit_signal("day_phase_updated", current_day_phase)
	if current_day_phase == DAY_STATE.DUSK:
		if _current_day_phase_time > _dusk_duration:
			current_day_phase = DAY_STATE.NIGHT
			emit_signal("day_phase_updated", current_day_phase)
	if current_day_phase == DAY_STATE.NIGHT:
		if _current_day_phase_time > _night_phase_duration + _dusk_duration:
			current_day_phase = DAY_STATE.DAWN
			_current_day_phase_time = 0.0
			emit_signal("day_phase_updated", current_day_phase)

	if _update_idle_time > 0.02:
		emit_signal("day_phase_time_updated", _current_day_phase_time)
		emit_signal("day_time_updated", _current_day_time)
		_update_idle_time = 0.0
