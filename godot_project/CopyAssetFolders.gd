...
func _init() -> void:
	...
	# Copy the default_options.cfg to the build folder.
	...
	dir.copy("default_controls.json", "{0}/default_controls.json".format([_globalized_build_path]))

	# Copy the default_state.json to the build folder.
	print("Copying 'default_context.json'")
	dir.copy("default_context.json", "{0}/default_context.json".format([_globalized_build_path]))

	quit()

func _copy_directory_content(current_folder : String) -> void:
	var dir : Directory = Directory.new()
